# BMI 식을 이용하여 원하는 양만큼의 자료를 생성하여 파일로 저장 후 분류모델 적용
# BMI(체질량 지수) = 몸무게(kg) / (키) * (키)
# print(60 / ((170 / 100) * (170 / 100)))
"""
import random
...
# 무작위로 데이터 생성
cnt = {'thin' : 0, 'normal': 0, 'fat' : 0} # 건수를 위한 dict type

random.seed(12)
for i in range(50000):
    h = random.randint(150, 200) # 키
    w = random.randint(35, 100) # 몸무게
    label = calc_bmi(h, w) # bmi지수에 따른 등급
    cnt[label] += 1
    fp.write('{0},{1},{2}\n'.format(h, w, label))
...
# label = label.map({'thin':0, 'normal' : 1, 'fat' : 2}) # 문자를 범주형 수치화 - 권장
# print(label.head(3))
...
# 분류 모델
model = svm.SVC().fit(data_train, label_train) # train data로 학습
pred = model.predict(data_test) # test data로 검정
