...
'''
df = pd.DataFrame({'id1' : (100,200,300,400,500), 'id2' : (-2,-3,-1,-7,-9)})
print(df)
plt.scatter(df.id1, df.id2)
plt.show()
print(df.cov()) # 공분산 : 관계를 표현하기에 수치가 일정하지 않음
print(df.corr()) # 피어슨 상관계수(r) : 공분산을 표준화해서 일정한 기준이 준비됨. |0.3| 이상일 때 작업을 시도
'''
# 일반적으로
...
print('\n공분산 출력----')
print(np.cov(data.친밀도, data.적절성)) # numpy는 2개씩밖에 비교를 못함
...
print(data.corr()) # 만족도와 적절성 : 뚜렷한 양적 선형관계
print(data.corr(method = 'pearson')) # 정규성 O, 변수가 등간, 비율 척도인 경우
