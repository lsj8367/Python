...
model = stats.linregress(x, y)  # 지상파 - 운동
print(model)
...
model2 = stats.linregress(x2, y2)  # 지상파 - 종편
...
# 지상파 시청시간이 늘어날 수록 종편시청 시간도 늘어나는 것을 확인.

jtime = float(input('지상파 입력시간 입력:'))
fit_value1 = np.polyval([model.slope, model.intercept], jtime)
fit_value2 = np.polyval([model2.slope, model2.intercept], jtime)
print('예측 운동시간 : ', fit_value1)
print('예측 종편시청 시간 : ', fit_value2)
