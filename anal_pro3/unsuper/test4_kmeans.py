# 비계층적 군집 방법
# K-means : 군집중심점이라는 임의 지점을 선택해 해당 중심에 가까운 자료들을 선택해 군집하는 방법
...
# KMeans는 n_clusters = ? 가 매우 중요한 요소
...
plt.scatter(kmodel.cluster_centers_[:, 0], kmodel.cluster_centers_[:, 1], s=250, c='black', marker='+', label='center') # 중심점 출력
plt.legend()
plt.grid(True)
plt.show()

# K값 찾기
# 방법1 : elbow 기법 - SSE(오차제곱합) 이용
def elbowFunc(x):
    sse = []
    for i in range(1, 11):
...
elbowFunc(x) # 그래프를 통해 최적의 클러스터 수는 3임을 알 수 있음

# 방법2 : silhoutte(실루엣) 기법 : 클러스터의 품질을 정량적으로 계산
'''
실루엣(silhouette) 기법
