...
import os
print(os.getcwd()) # 현재 경로표시

try:
    print('file 읽기')
    # f1 = open("C:/work/psou/pro1/pack2/ftest.txt", mode='r', encoding='utf-8')
    ...
    # print(f4.readline())
    lines = f4.readlines() # 모든행을 읽혀서 변수가 기억 type은 list
    # print(lines)
    print(lines[0])
    print(lines[1:5])
...
print('\n\n피클링(복합 객체 처리) - object type으로 파일 i/o')
import pickle
try:
    ...
    with open('fpickle.dat', 'wb') as f3:
        pickle.dump(tupleData, f3) # 복합객체 저장 (대상, 파일객체명)
        pickle.dump(listData, f3) # 리스트 타입만 저장(집합형)
    print('pickle을 이용한 저장 완료')
...
print("\n동이름으로 우편번호 찾기, 나머지 주소 찾기====")
try:
    dong = input('동이름 입력:')
    # dong = '역삼'
    # print(dong)
    
    with open('zipcode.txt', mode='r', encoding = 'utf-8') as zipf:
        line = zipf.readline()
        # print(line)
        
        while line:
            # lines = line.split('\t')
            lines = line.split(chr(9)) # \t 와 같은 chr(9)
