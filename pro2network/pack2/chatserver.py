...
ss.bind(('',5555)) # 192.168.0.135
ss.listen(5)
print("채팅 서버 서비스 시작....")
users = []

def ChatUser(conn): # 실제 채팅 진행하는 함수
    name = conn.recv(1024)
    data = '^^ ' + name.decode('utf-8') + '님 입장!!!'
    print(data)
    try:
        for p in users:
            p.send(data.encode('utf-8')) # 모든 채팅 사용자에게 입장을 알림
        ...
        while True:
            ...
            for p in users:
                p.send(data.encode('utf-8')) # 모든 채팅 사용자에게 글을 전송
            
            
    except: # 채팅 사용자가 채팅방을 빠져 나간경우
